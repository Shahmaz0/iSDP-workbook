...
class AddEditEmojiViewController: UITableViewController {
    ...
    func updateSaveButtonSafe() {
//        let symbolText = symbolTextField.text ?? ""
        let nameText = nameTextFiield.text ?? ""
        let descriptionText = descriptionTextField.text ?? ""
        let usageText = usageTextField.text ?? ""
        saveButton.isEnabled =
        containSingleEmoji(symbolTextField) &&
            !nameText.isEmpty &&
    ...
    func containSingleEmoji(_ textField: UITextField) -> Bool {
        ...
        let isCombineIntoEmoji = text.unicodeScalars.count > 1 && text.unicodeScalars.first?.properties.isEmoji ?? false
        
        let isPresented = text.unicodeScalars.first?.properties.isEmojiPresentation ?? false
        
        return isCombineIntoEmoji || isPresented
    }
